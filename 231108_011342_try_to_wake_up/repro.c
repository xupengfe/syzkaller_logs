...
{
  intptr_t res = 0;
  memcpy((void*)0x20000100, "/sys/fs/cgroup", 14);
  res = syscall(__NR_openat, /*fd=*/0xffffffffffffff9cul, /*dir=*/0x20000100ul,
                /*flags=*/0ul, /*mode=*/0ul);
  if (res != -1)
    r[0] = res;
  ...
  *(uint64_t*)0x20000078 = 0;
  syscall(__NR_perf_event_open, /*attr=*/0x20000000ul, /*fd=*/r[0], /*cpu=*/0ul,
          /*group=*/-1, /*flags=*/0xdul);
  res = -1;
  res = syz_open_dev(/*dev=*/0xc, /*major=*/4, /*minor=*/0x15 + procid * 2);
  if (res != -1)
    r[1] = res;
  *(uint32_t*)0x20000040 = 1;
  *(uint32_t*)0x20000044 = 4;
  *(uint32_t*)0x20000048 = 0;
  *(uint32_t*)0x2000004c = 0x1c;
  *(uint32_t*)0x20000050 = 0x408;
  *(uint64_t*)0x20000058 = 0;
  syscall(__NR_ioctl, /*fd=*/r[1], /*cmd=*/0x4b72, /*arg=*/0x20000040ul);
}
int main(void)
{
